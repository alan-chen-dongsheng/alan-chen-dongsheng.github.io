...
tags:
  ...
  - Vulkan
draft: false
summary: '本文档记录从下载 ONNX 模型，到固化 Shape，再到转换为 MLIR IR，编译为 VMFB，最后执行推理的完整流程。 **编译工具**: 全部使用本地编译的 IREE，不依赖外部安装包。 安装本地 Runtime 绑定： ONNX 模型的输入通常使用动态 batch size（用 `N`...'
---
